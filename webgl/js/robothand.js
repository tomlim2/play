...
var axes = true;
var ground = true;
var arm, forearm, body, lefthand, righthand;
...
function fillScene() {
	...
	createRobotGrabber( righthand, handlength, robotBodyMaterial );
	righthand.position.y = faLength;
	righthand.position.z = 10;
	forearm.add(righthand);

	var handlength = 38;
	lefthand = new THREE.Object3D();
	createRobotGrabber( lefthand, handlength, robotBodyMaterial );
	lefthand.position.y = faLength;
	lefthand.position.z = -10;
	forearm.add(lefthand);

}

function createRobotGrabber ( part, handlength, material) {
	var box = new THREE.Mesh( new THREE.CubeGeometry(30,handlength,4), material);
	
	box.position.y = handlength/2;
	part.add(box)
	// body...
}
...
function init() {
	...
	camera.position.set(-362, 247, 40);
	cameraControls.target.set(-13, 80, 2);
	fillScene();
...
function render() {
	...
	// lefthand.rotation.z = effectController.hz * Math.PI/180;
